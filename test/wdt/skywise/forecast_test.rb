...
class Wdt::Skywise::ForecastTest < Minitest::Test
  ...
  def test_weather_for_without_parameters
    stub_request(:any, /#{Wdt::Skywise::Forecast::Client.base_url}/).to_return(status: 200, body: "{}")
    client = Wdt::Skywise::Forecast::Client.new
    response = client.weather_for({})
    assert_equal false, response.success
    assert_equal "No parameters specified", response.error
    assert_nil response.response
  end

  def test_weather_for_by_incomplete_lat_no_long
    stub_request(:any, /#{Wdt::Skywise::Forecast::Client.base_url}/).to_return(status: 200, body: "{}")
    client = Wdt::Skywise::Forecast::Client.new
    response = client.weather_for(LAT: '32128')
    assert_equal false, response.success
    assert_equal "LONG must be provided with LAT", response.error
    assert_nil response.response
  end

  def test_weather_for_by_incomplete_long_no_lat
    stub_request(:any, /#{Wdt::Skywise::Forecast::Client.base_url}/).to_return(status: 200, body: "{}")
    client = Wdt::Skywise::Forecast::Client.new
    response = client.weather_for(LONG: '32128')
    assert_equal false, response.success
    assert_equal "LAT must be provided with LONG", response.error
    assert_nil response.response
  end

  def test_weather_for_by_city_with_no_state_or_country
    stub_request(:any, /#{Wdt::Skywise::Forecast::Client.base_url}/).to_return(status: 200, body: "{}")
    client = Wdt::Skywise::Forecast::Client.new
    response = client.weather_for(CITY: '32128')
    assert_equal false, response.success
